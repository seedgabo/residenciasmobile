...
export class SurveyComponent implements AfterViewInit, OnDestroy {
  ...
  ngAfterViewInit() {
    if (this.chart) { this.chart.clear(); this.chart.destroy() }
    this.renderChart();
    this.getVote();
    funct = (data) => {
      this.update();
  ...
  getVote() {
    this.api.get(`votes?where[user_id]=${this.api.user.id}&where[survey_id]=${this._survey.id}`)
  ...
  postVote(data) {
    if (!data) return;
    this.api.post('votes', { response: data, user_id: this.api.user.id, survey_id: this._survey.id })
      .then((data) => {
        console.log(data);
        this.vote = data;
        this.update();
  ...
  update() {
    this.api.get('surveys/' + this._survey.id)
      .then((survey) => {
        this._survey = survey;
        this.renderChart();
      })
