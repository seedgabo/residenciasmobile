...
export class InvoicesPage {
  ...
  getInvoices(refresher = null) {
    this.api
      .get(
        `invoices?order[date]=desc&orWhere[residence_id]=${
  ...
  openMenu(invoice) {
    var buttons = [
      {
        text:
          this.api.trans("literals.download") +
          " " +
          this.api.trans("literals.invoice"),
        icon: "document",
        cssClass: "danger",
        handler: () => {
      ...
      {
        text:
          this.api.trans("literals.view_resource") +
          " " +
          this.api.trans("literals.details"),
        icon: "ios-list-box",
        cssClass: "",
        handler: () => {
    ...
    if (invoice.receipts && invoice.receipts.length > 0) {
      buttons.push({
        text:
          this.api.trans("literals.download") +
          " " +
          this.api.trans("literals.receipt"),
        icon: "filing",
        cssClass: "secondary",
        handler: () => {
