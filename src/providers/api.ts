    ...
    this.ready.then(() => {
      ...
      this.Echo.private("Application")
        ...
        .listen("VisitCreated", data => {
          ...
          this.zone.run(() => {
            if (this.visits) this.visits.unshift(data.visit);
        ...
        .listen("EventUpdated", data => {
          ...
          this.zone.run(() => {
            if (event_index > -1) this._events[event_index] = data.event;
          });
          this.events.publish("events:changed", {});
        })
  ...
  public PetChanged(data) {
    if (this.objects.pets) {
      ...
      this.zone.run(() => {
        ...
        if (data.residence) pet.residence = data.residence;
      });
    }
  }
